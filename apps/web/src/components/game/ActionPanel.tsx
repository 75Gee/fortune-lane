...
export function ActionPanel({ game, playerId, onCommand, busy = false, waitingLabel = '行动进行中…', onSkipToLive, onManageAssets, onWatch }: { game: GameView; playerId: string; onCommand: (command: GameCommand) => void } & { busy?: boolean; waitingLabel?: string; onSkipToLive?: (() => void) | undefined; onManageAssets: () => void; onWatch?: (() => void) | undefined }) {
  ...
  if (game.pendingDecision?.playerId === playerId) return <PropertyDecision key={`${game.turnNumber}:${game.pendingDecision.tileIndex}:${game.pendingDecision.type}`} game={game} playerId={playerId} onCommand={onCommand} />
      ...
      return (
        <div className="turn-action">
          ...
          <div className="action-grid">
            ...
            <button disabled={!payment.allowed} title={payment.reason ?? undefined} onClick={() => onCommand({ type: 'PAY_JAIL_FINE', stockFunding: payment.stockFunding })}><Banknote size={18} /> {payment.stockFunding && payment.allowed ? '卖股并支付' : '支付'} {money(JAIL_FINE)}</button>
  ...
  if (game.phase === 'WAITING_FOR_DEBT' && game.pendingDebt) {
    ...
    return (
      <div className="turn-action debt-action">
        ...
        {payment.allowed && payment.stockFunding ? <StockPaymentHint payment={payment} /> : <p>{difference > 0 ? `还差 ${money(difference)}，可卖股、卖房或抵押筹款` : '钱已凑齐，可以付款了'}</p>}
        <div className="decision-buttons">
          <button className="accept" disabled={!payment.allowed} onClick={() => onCommand(payment.stockFunding ? { type: 'LIQUIDATE_ASSETS', selections: [], ...payment.stockFunding } : { type: 'SETTLE_DEBT' })}><CircleDollarSign size={18} /> {payment.stockFunding && payment.allowed ? '卖股并支付欠款' : '支付欠款'}</button>
          <button onClick={onManageAssets}><Building2 size={18} />筹款 / 管理资产</button>
        </div>
        <button className="debt-give-up" onClick={() => onCommand({ type: 'DECLARE_BANKRUPTCY' })}>放弃筹款，宣告破产</button>
      </div>
    )
  }
