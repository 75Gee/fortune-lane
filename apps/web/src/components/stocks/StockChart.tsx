...
export function StockChart({ points, color, name, costCents }: { points: readonly StockPricePoint[]; color: string; name: string; costCents?: number | undefined }) {
  ...
  const x = (index: number) => left + (points.length === 1 ? .5 : index / (points.length - 1)) * plotWidth
  ...
  return <figure className="stock-chart">
    ...
    <svg viewBox={`0 0 ${width} ${height}`} role="slider" tabIndex={0} aria-label={`${name}价格走势，左右方向键查看历史报价`} aria-valuemin={0} aria-valuemax={points.length - 1} aria-valuenow={selected} aria-valuetext={`第 ${point.turn} 回合，${stockPrice(point.priceCents)}`}
      onPointerMove={event => { const rect = event.currentTarget.getBoundingClientRect(); setCursor(Math.max(0, Math.min(points.length - 1, Math.round(((event.clientX - rect.left) / rect.width * width - left) / plotWidth * (points.length - 1))))) }}
...
export function StockSparkline({ points, color }: { points: readonly StockPricePoint[]; color: string }) {
  ...
  return <svg viewBox="0 0 100 25" aria-hidden="true"><path d={prices.map((price, index) => `${index ? 'L' : 'M'}${index / (prices.length - 1) * 100},${23 - (price - low) / range * 21}`).join(' ')} fill="none" stroke={color} strokeWidth="1.5" strokeLinejoin="round" /></svg>
}
